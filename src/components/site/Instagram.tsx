...
import { Instagram as InstagramIcon, ExternalLink, Heart, MessageCircle, Upload } from "lucide-react";
...
export function InstagramGallery() {
  ...
  const getSpan = (index: number) => {
    switch (index) {
      case 0: return "row-span-2 col-span-1"; // Slot 1: Tall (spans 2 rows on all devices)
      case 3: return "row-span-2 col-span-1"; // Slot 4: Tall (spans 2 rows on all devices)
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <div className="flex flex-col items-center text-center mb-12 sm:mb-16">
          <div className="flex items-center gap-2 mb-4 opacity-60">
             <span className="text-[10px] uppercase tracking-[0.4em] font-bold text-cocoa">— @CHRISTFITZ</span>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-[18px] auto-rows-[200px] sm:auto-rows-[280px]">
            {showSkeletons ? (
              ...
              <div className="col-span-full row-span-2 py-32 text-center rounded-[3rem] bg-cocoa/5 border border-dashed border-cocoa/20 flex flex-col items-center justify-center">
                 ...
                 <p className="text-cocoa/40 font-medium max-w-md px-10">Gallery is ready for your vision. Head to Admin to upload your high-quality pieces.</p>
              </div>
            )}
          </div>
