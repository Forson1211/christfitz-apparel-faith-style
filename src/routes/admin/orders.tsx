...
const statusColor = (s: string) => {
  switch (s) {
    ...
    case "pending":
      return "bg-gold/20 text-yellow-700";
    case "cancelled":
      return "bg-red-500/10 text-red-600";
    case "refunded":
      return "bg-purple-500/10 text-purple-600";
    default:
      return "bg-cocoa/5 text-cocoa/60";
  }
};
    ...
    <div className="w-full space-y-6 pb-10">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          >
            <div className="text-2xl font-display font-bold text-cocoa">
              {loading ? "—" : value}
            </div>
        ...
        {loading ? (
          ...
          <div className="py-20 text-center text-cocoa/40 text-sm">
            {search
              ...
              : "No orders yet. Orders appear here when customers place them."}
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-cocoa/5">
                {filtered.map((order) => (
                  <tr key={order.id} className="hover:bg-white/30 transition-colors">
                    <td className="px-6 py-4 font-bold text-cocoa font-mono text-xs">
                      {order.id.slice(0, 8).toUpperCase()}
                    </td>
                    <td className="px-6 py-4 text-cocoa/70 text-xs">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${statusColor(order.status)}`}
                      >
                        {order.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right font-bold text-cocoa">
          ...
          >
            <div className="p-6 bg-cocoa text-cream flex items-center justify-between">
              <div>
                ...
                <p className="text-cream/60 text-xs mt-0.5 font-mono">
                  {editing.id.slice(0, 8).toUpperCase()}
                </p>
              </div>
              <button onClick={() => setEditing(null)} className="p-1 hover:text-gold transition">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  onClick={() => saveStatus(editing.id, editing.status)}
                  disabled={saving}
