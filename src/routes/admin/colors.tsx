  ...
  return (
    <div className="max-w-4xl space-y-10 pb-20">
      <div>
        ...
        <p className="mt-2 text-muted-foreground">Customize your store's visual identity to match your brand's spirit.</p>
      </div>
      ...
      <section className="rounded-[2.5rem] glass shadow-luxe overflow-hidden border border-white/40">
        ...
        <div className="p-8 grid gap-8 sm:grid-cols-2">
          {(["cocoa","coffee","cream","gold"] as const).map((k) => (
            <div key={k} className="p-6 rounded-3xl bg-cocoa/5 border border-cocoa/10 transition-all hover:bg-cocoa/[0.07]">
              <div className="flex items-center justify-between mb-4">
                <label className="text-[10px] uppercase tracking-widest text-cocoa/40 font-bold">{k} Color</label>
                <div 
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <section className="rounded-[2.5rem] glass shadow-luxe overflow-hidden border border-white/40">
          <div className="p-8 border-b border-cocoa/5 flex items-center justify-between">
            ...
            <button onClick={() => save("logo", logo)} disabled={busy} className="text-gold font-bold text-xs uppercase tracking-widest hover:translate-x-1 transition-transform">
              Save Logo
            </button>
          </div>
          <div className="p-8 space-y-6">
            <div className="flex flex-col items-center justify-center p-10 rounded-3xl bg-cocoa/5 border-2 border-dashed border-cocoa/10 relative group">
              {logo.url ? (
                <img src={logo.url} alt="logo" className="h-24 w-24 rounded-full object-cover shadow-soft" />
              ) : (
              ...
              />
              <p className="mt-4 text-[10px] uppercase tracking-widest text-cocoa/40 font-bold">Click to upload</p>
            </div>
            <div className="space-y-1.5">
              <label className="text-[10px] uppercase tracking-widest text-cocoa/50 font-bold px-1">Fallback Symbol</label>
              <input 
                value={logo.symbol} 
            ...
            {logo.url && (
              <button onClick={() => setLogo({ ...logo, url: "" })} className="w-full text-center text-xs font-bold text-red-500 hover:text-red-600 transition-colors">
                Remove Logo Image
              </button>
            )}
          </div>
        ...
        <section className="rounded-[2.5rem] glass shadow-luxe overflow-hidden border border-white/40">
          <div className="p-8 border-b border-cocoa/5 flex items-center justify-between">
            ...
            <button onClick={() => save("favicon", favicon)} disabled={busy} className="text-gold font-bold text-xs uppercase tracking-widest hover:translate-x-1 transition-transform">
              Save Favicon
            </button>
          </div>
          <div className="p-8 space-y-6">
            <div className="flex flex-col items-center justify-center p-10 rounded-3xl bg-cocoa/5 border-2 border-dashed border-cocoa/10 relative group">
              ...
              <input 
                ...
                className="absolute inset-0 opacity-0 cursor-pointer" 
              />
              <p className="mt-4 text-[10px] uppercase tracking-widest text-cocoa/40 font-bold">Upload ICO/PNG</p>
            </div>
            <p className="text-xs text-cocoa/40 text-center px-4">The favicon appears in the browser tab and search results.</p>
          </div>
        </section>
      </div>
    </div>
