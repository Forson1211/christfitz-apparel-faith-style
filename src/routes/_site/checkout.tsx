  ...
  if (items.length === 0 && step !== 3) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center text-center p-6">
        ...
        <button onClick={() => navigate({ to: "/products" })} className="mt-6 text-gold hover:underline">
          Go back to shop
        </button>
      </div>
    );
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center p-6 text-center">
        ...
        <p className="mt-4 max-w-md text-muted-foreground text-lg">
          Your order has been received. We've sent a confirmation email to <span className="text-cocoa font-medium">{formData.email}</span>.
        </p>
        <button
          onClick={() => navigate({ to: "/" })}
        ...
        <div className="grid gap-12 lg:grid-cols-[1.7fr_1fr]">
          ...
          <div className="space-y-8">
            ...
            <div className="flex items-center gap-4 border-b border-cocoa/10 pb-6">
              <div className={`flex h-10 w-10 items-center justify-center rounded-full font-display ${step >= 1 ? 'bg-cocoa text-cream' : 'bg-cocoa/10 text-cocoa'}`}>1</div>
              <span className="font-display text-xl">Shipping</span>
              <div className="h-px w-12 bg-cocoa/10" />
              <div className={`flex h-10 w-10 items-center justify-center rounded-full font-display ${step >= 2 ? 'bg-cocoa text-cream' : 'bg-cocoa/10 text-cocoa'}`}>2</div>
            ...
            {step === 1 ? (
              ...
              >
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <label className="text-xs uppercase tracking-widest text-cocoa/60 font-bold">First Name</label>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-2">
                    <label className="text-xs uppercase tracking-widest text-cocoa/60 font-bold">Last Name</label>
                    <input 
                      type="text" 
                ...
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-widest text-cocoa/60 font-bold">Email Address</label>
                  <input 
                    type="email" 
                ...
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-widest text-cocoa/60 font-bold">Phone Number</label>
                  <input 
                    type="tel" 
                ...
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-widest text-cocoa/60 font-bold">Shipping Address</label>
                  <input 
                    type="text" 
                ...
                <div className="space-y-2">
                  <label className="text-xs uppercase tracking-widest text-cocoa/60 font-bold">City</label>
                  <input 
                    type="text" 
                    value={formData.city}
              ...
              >
                <div className="space-y-4">
                  <label className="text-xs uppercase tracking-widest text-cocoa/60 font-bold">Select Payment Method</label>
                  ...
                  >
                    ...
                    <div className={`h-5 w-5 rounded-full border-2 flex items-center justify-center ${formData.paymentMethod === 'momo' ? 'border-gold' : 'border-cocoa/20'}`}>
                      {formData.paymentMethod === 'momo' && <div className="h-2.5 w-2.5 rounded-full bg-gold" />}
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <div className={`h-5 w-5 rounded-full border-2 flex items-center justify-center ${formData.paymentMethod === 'card' ? 'border-gold' : 'border-cocoa/20'}`}>
                      {formData.paymentMethod === 'card' && <div className="h-2.5 w-2.5 rounded-full bg-gold" />}
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-2xl bg-cocoa/5 border border-cocoa/10 p-6 space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-xs leading-relaxed text-cocoa/70">
                      Your payment is secured by our 256-bit SSL encrypted system. For Momo payments, you will receive a prompt on your phone after clicking place order.
                    </p>
                  </div>
                </div>
            ...
            <div className="rounded-3xl glass p-8">
              ...
              <div className="space-y-4 mb-8 max-h-[300px] overflow-auto pr-2">
                {items.map(item => (
                  <div key={item.id} className="flex gap-4">
                    ...
                    <p className="text-sm font-medium self-center tabular-nums">₵{(item.price * item.quantity).toFixed(2)}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-3 border-t border-cocoa/10 pt-6">
                <div className="flex justify-between text-sm text-cocoa/60">
                  <span>Subtotal</span>
                  <span className="tabular-nums font-medium text-cocoa">₵{subtotal.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm text-cocoa/60">
                  <span>Shipping</span>
                  <span className="tabular-nums font-medium text-cocoa">{shipping === 0 ? "FREE" : `₵${shipping.toFixed(2)}`}</span>
                </div>
